    ...
    public class AccountService : IAccountService
    {
        ...
        public async Task<Response<PasswordVM>> ChangePasswordAsync(PasswordVM model)
        {
            try
            {
                ...
                return new Response<PasswordVM>
                (
                    model,
                    result.Succeeded ? "Password changed successfully" : "Error changing password",
                    !result.Succeeded
                );
            }
            catch (Exception ex)
        ...
        Task<(Response<ApplicationUser>, string Role)> IAccountService.GetRole(string Email)
        {
            try
            {
                ...
                if (user == null)
                {
                    return Task.FromResult((new Response<ApplicationUser>(null, "User not found", true)
                   , string.Empty));
                }
                var role = _userManager.GetRolesAsync(user).Result.FirstOrDefault();
                return Task.FromResult((new Response<ApplicationUser>
                (
                    (ApplicationUser)user, "Role retrieved successfully",
                    false
