    ...
    public class WorkoutPlanService : IWorkoutPlanService
    {
        ...
        public async Task<Response<WorkoutPlanVM>> CreateWorkoutPlanAsync(WorkoutPlanVM workoutPlanVm)
        {
            try
            {
                ...
                {
                    workoutPlan.WorkoutPlanItems = Mapper.Map<List<WorkoutPlanItem>>(workoutPlanVm.workoutPlanItemVMs.Where(e => !string.IsNullOrWhiteSpace(e.ExerciseName)));
                }
        ...
        public async Task<Response<WorkoutPlanVM>> GetWorkoutPlanByIdAsync(int workoutPlanId)
        {
            try
            {
                ...
                if (workoutPlan != null)
                {
                    ...
                    workoutPlanVm.workoutPlanItemVMs = Mapper.Map<List<WorkoutPlanItemVM>>(workoutPlan.WorkoutPlanItems?.ToList() ?? new List<WorkoutPlanItem>());
                    return new Response<WorkoutPlanVM>(workoutPlanVm, null, false);
                }
                else
                {
